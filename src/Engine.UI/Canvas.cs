...
public sealed class Canvas : Panel
{
    ...
    public void PlacePopup(Popup popup, Vector2 pointerPosition = default)
    {
        ...
        var anchorBounds = placement == PopupPlacement.Pointer
            ? new UIClipRect(pointerPosition.X, pointerPosition.Y,
                pointerPosition.X, pointerPosition.Y)
            : owner is null
                ? new UIClipRect(0f, 0f, 0f, 0f)
                : new UIClipRect(owner.Left, owner.Top, owner.Right, owner.Bottom);
        var result = PopupPlacementResolver.Resolve(
            anchorBounds,
            size,
            placement,
            popup.PlacementOffset,
            workArea,
            popup.ConstrainToOverlayBounds,
            owner is not null);
        popup.ActualPlacement = result.Placement;
        SetPosition(popup, result.Position);
    }
