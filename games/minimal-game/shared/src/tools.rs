...
pub fn register(builder: AppBuilder) -> io::Result<(AppBuilder, ToolExtensions)> {
    ...
    let tool = Tool::new("game_state", "Read the minimal game's latest owned snapshot, published after Update. No world access occurs on the tooling thread.",
        ...
        .with_raw_output_schema(json!({"type":"object","required":["fixed_updates","frame_updates","stamina","coins","movement_quest_completed","entity_count"],
            "properties":{"fixed_updates":{"type":"integer"},"frame_updates":{"type":"integer"},"stamina":{"type":"integer"},
    ...
    tools.register(tool, move |arguments| {
        ...
        snapshot.lock().expect("game snapshot lock poisoned").clone().map(CallToolResult::structured)
...
struct SnapshotPlugin(Arc<Mutex<Option<Value>>>);
...
impl Plugin for SnapshotPlugin {
    fn build(&self, builder: &mut AppBuilder) -> RuntimeResult<()> {
        ...
        *published.lock().expect("game snapshot lock poisoned") = Some(value);
        Ok(())
    });
        Ok(())
    }
...
mod tests {
    ...
    fn first_tooling_snapshot_includes_the_completed_game_update() -> RuntimeResult<()> {
        ...
        let first = snapshot.lock().unwrap().clone().unwrap();
        assert_eq!(first["frame_updates"], 1);
        assert_eq!(first["entity_count"], 1);
        app.tick(Duration::from_millis(17))?;
        assert_eq!(
            snapshot.lock().unwrap().as_ref().unwrap()["frame_updates"],
            2
        );
        assert_eq!(first["frame_updates"], 1);
        app.shutdown()
